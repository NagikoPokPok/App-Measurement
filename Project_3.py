...
# This exponentiates the predictions (undoing the log transform)
print("\nConverting a few examples back to original scale (person-months):")
sample_indices = y_test.index[:5]
for idx in sample_indices:
    actual = np.exp(y_log[idx]) - 1  # Reverse log1p transformation
    cocomo = np.exp(predictions['COCOMO II'][y_test.index.get_loc(idx)]) - 1
    best = np.exp(predictions[best_model][y_test.index.get_loc(idx)]) - 1
    print(f"Sample {idx}: Actual={actual:.1f}, COCOMO II={cocomo:.1f}, {best_model}={best:.1f}")