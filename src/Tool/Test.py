...
def create_effort_input_form(method, all_columns):
    input_data = {}
    
    if method == 'LOC':
        # Additional constants for COCOMO calculation
        input_data['a'] = st.sidebar.number_input('Constant (a)', value=2.94, format="%.2f")
        input_data['b'] = st.sidebar.number_input('Size Exponent (b)', value=0.91, format="%.2f")
        ...
        # Additional fields for LOC estimation
        ...
        # Add project mode dropdown for complexity adjustment
        input_data['model'] = st.sidebar.selectbox(
            'Select Project Mode',
            ['embedded', 'organic', 'semidetached']
        )
...
def display_results(pred_effort):
    ...
    st.write(f"### Estimated Effort : {pred_effort:.2f}")
...
def main():
    ...
    # Load the columns from your trained model
    all_columns = model_loc.feature_names_in_  # This will give you the feature names used in training the model
    ...
    data = create_effort_input_form(method, all_columns)
